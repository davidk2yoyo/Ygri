    ...
    if (type === "image") {
      ...
      return `
        <div class="content-block" style="break-inside:avoid;page-break-inside:avoid;">
          <img src="${esc(content.url)}" alt="${esc(content.caption || "")}" crossorigin="anonymous" data-pdf
            ...
            <tbody>
              ${items.map((d, i) => {
                ...
                const photoCell = hasPhotos
                  ? (d.photo_url ? `<td style="padding:4px;"><img src="${esc(d.photo_url)}" crossorigin="anonymous" data-pdf style="width:72px;height:56px;object-fit:cover;border-radius:4px;" /></td>` : "<td></td>")
                  : "";
                return `
                  <tr class="${i % 2 === 0 ? "even" : "odd"}">
                    ${photoCell}
...
window.onload = async function() {
  const MAX_PX = 1400;
  const QUALITY = 0.82;
  const imgs = Array.from(document.querySelectorAll('img[data-pdf]'));

  const compress = (img) => new Promise((resolve) => {
    const process = () => {
      try {
        if (!img.naturalWidth) { resolve(); return; }
        const ratio = Math.min(1, MAX_PX / Math.max(img.naturalWidth, img.naturalHeight));
        if (ratio >= 1) { resolve(); return; }
    ...
    if (img.complete) { process(); } else { img.onload = process; img.onerror = resolve; }
  });

  await Promise.all(imgs.map(compress));
  window.print();
};
<\/script>
</body>

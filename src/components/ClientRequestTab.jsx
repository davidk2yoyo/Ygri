...
export default function ClientRequestTab({ trackId, clientName, projectName }) {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (existing) {
        ...
        setTeamNotes(existing.team_notes || "");
        setSummary({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.from("client_requests")
        .update({ raw_text: rawText, links, team_notes: teamNotes })
  ...
  const generateSummary = async () => {
    ...
    try {
      ...
      const { data: updated, error } = await supabase.from("client_requests")
        .update({ raw_text: rawText, links, team_notes: teamNotes, ...nextSummary, summary_generated_at: new Date().toISOString() })
        .eq("id", req.id).select().single();
      if (error) throw error;
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {/* Team notes / direction — manual, not AI-generated */}
      <div className="bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-700/40 rounded-2xl p-5">
        <h3 className="font-bold text-darkblack-700 dark:text-white flex items-center gap-2 mb-2">
          📌 Team Notes / Instructions
        </h3>
        <p className="text-xs text-bgray-500 dark:text-bgray-400 mb-2">
          Extra direction for the team — e.g. "quote with this supplier", "check this idea first". Not generated by AI, written by you.
        </p>
        <textarea
          rows={3}
          value={teamNotes}
          onChange={e => setTeamNotes(e.target.value)}
          placeholder="e.g. Quote this with ChuanXin first, they already gave us a good price on similar items..."
          className="w-full px-3 py-2 border border-amber-200 dark:border-amber-700/50 rounded-lg text-sm bg-white dark:bg-darkblack-600 text-darkblack-700 dark:text-white focus:ring-2 focus:ring-amber-400 placeholder-bgray-400 resize-y"
        />
      </div>
        ...
        >
          ...
          {teamNotes.trim() && (
            <div style={{ marginBottom: "18px", backgroundColor: "#fffbeb", border: "1px solid #fde68a", borderRadius: "8px", padding: "12px 14px" }}>
              ...
              <div style={{ fontSize: "13px", color: "#78350f", whiteSpace: "pre-wrap" }}>{teamNotes}</div>
            </div>
          )}

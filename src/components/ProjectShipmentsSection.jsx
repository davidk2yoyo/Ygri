...
function getCarrierBadge(carrier) {
  ...
  if (c.includes("china ems") || c.includes("ems")) return { abbrev: "EM", colorClass: "bg-green-600 text-white" };
...
function get17TrackCode(carrier) {
  ...
  if (c.includes("zto"))        return 190175;
  ...
  if (c.includes("j&t"))        return 100295;
...
function buildTrackItem(trackingNumber, carrier, carrierParam) {
  ...
  if (carrierParam && needsPhoneParam(carrier)) item.phone_number_last_4 = carrierParam;
  return item;
}
            ...
            <form onSubmit={handleSave} className="px-6 py-5 space-y-4">
              ...
              {needsPhoneParam(form.carrier) && (
                <div>
                  ...
                  />
                  <p className="text-[11px] text-bgray-400 mt-1">Required for domestic {form.carrier} — last 4 digits of recipient phone</p>
                </div>
              )}
